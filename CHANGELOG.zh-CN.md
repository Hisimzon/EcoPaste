...
- 修复使用 Alt 全局快捷键时触发 Windows 菜单按键提示的问题。
- 修复备注输入框首次明确点击后仍需再次点击才能输入的问题，同时保持窗口无焦点唤起行为。
- 修复剪贴板编辑窗口意外隐藏及列表顶部滚动空白问题。

## [1.1.3](https://github.com/EcoPasteHub/EcoPaste/compare/v1.1.2...v1.1.3) (2026-08-07)

### 🐛 问题修复

- 修复 Windows 剪贴板窗口反复显示、隐藏及长时间后台运行后的生命周期异常。
- 修复打开偏好设置后剪贴板主窗口未隐藏的问题。
- 修复清空记录确认框抢占外部焦点或导致剪贴板主窗口隐藏的问题。

## [1.1.2](https://github.com/EcoPasteHub/EcoPaste/compare/v1.1.0...v1.1.2) (2026-08-07)

### ✨ 新功能

- 新增 Windows 不抢占焦点的拼音搜索，并支持手动点击搜索框后使用输入法。
- 将轻量模式和窗口延迟销毁扩展到剪贴板主窗口。
- 新增备注存在时清理正文冗余拼音索引的功能。

### 🐛 问题修复

- 修复唤起 EcoPaste 和粘贴记录时目标窗口焦点丢失的问题。
- 修复文本预览换行、面板尺寸及右侧空白异常。
- 修复清空或删除记录后剪贴板列表未立即刷新的问题。
- 禁用偏好设置原生右键菜单，并改进 Windows 列表项菜单生命周期。
- 跳过不存在的 Windows 系统自启动项，并统一 Tauri 日志插件版本。

### ⚡️ 性能优化

- 将拼音生成和历史索引回填改为后台分批处理。
- 减少虚拟列表请求、删除补位刷新、滚动卡顿及悬停预览开销。
